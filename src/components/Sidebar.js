...
export default function Sidebar() {
  ...
  return (
    <aside className={`bg-[#22314F] text-white ${collapsed ? 'w-20' : 'w-72'} py-4 h-[88vh] flex flex-col rounded-2xl m-4 transition-all duration-300 relative`}>
        ...
        {mainNavItems.map((item) => {
          ...
          return (
            <Link href={item.href} key={item.label}>
              <div
                className={`flex items-center py-3 cursor-pointer mx-4 hover:bg-[#2ED6A1] hover:text-[#22314F] rounded-full mb-1 transition-all duration-200 ${
      ...
      <button className={`m-6 mt-auto bg-[#2ED6A1] text-[#22314F] py-2 rounded-lg flex items-center justify-center gap-2 transition-all duration-200 ${collapsed ? 'justify-center px-0' : ''}`}>
        <FaSignOutAlt />
